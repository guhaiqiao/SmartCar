            ...
            with self.lock:
                if head[0] != self.srcID or head[1] != PCID:   #地址不符合则丢弃该包
                    ...
                    if head[7] == 3:   #判断是否为SYN包
                        ...
                        self.car.read(head[8])
                        self.car.write(bytes([PCID, self.srcID, ack % 256, ack // 256, seq % 256, seq // 256, 0, 0, 0, 0]))   #发ACK包
                elif head[8] != 0 and int.from_bytes(head[4:6], byteorder = "little") != ack + 1:   #新包情形或同时发包情形
                    ...
                    seq += 1
                    self.car.write(bytes([PCID, self.srcID, ack % 256, ack // 256, seq % 256, seq // 256, 0, 0, 0, 0]))   #发ACK包
                elif head[8] != 0 and int.from_bytes(head[4:6], byteorder = "little") == ack + 1:   #ACK包丢失情形
                    data = self.car.read(head[8])   #校验-存储-ACK
                    if head[9] != verify(data):
                        continue
                    recvbuf.append(data)
...
def send_once(car, dstID, lock, flags):
    global recvbuf, sendbuf, seq, ack, PCID, sending_flag, timer
    with lock:
        car.write(bytes([PCID, dstID, ack % 256, ack // 256, seq % 256, seq // 256, 0, flags, len(sendbuf[0]), 0]) + sendbuf[0])
        sending_flag = True
